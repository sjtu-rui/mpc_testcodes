% a nonlinear CSTR using LMPC and NMPC.
% model:
%       dcdt = F0*(c0 - c)/V - rate;
%       dTdt = F0*(T0 - T)/V - DeltaH/rhoCp*rate + 2U/(r*rhoCp)*(Tc - T); 
%       dhdt = (F0 - F)/A;
%       
% Date: 2020/08/20
% 
...
ode_casadi = mpc.getCasadiFunc(ode,[Nx, Nu, Np],{'x', 'u', 'p'},{'ode'});
